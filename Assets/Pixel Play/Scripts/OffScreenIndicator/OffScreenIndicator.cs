...
public class OffScreenIndicator : MonoBehaviour
{
    ...
    void DrawIndicators()
    {
        foreach(Target target in targets)
        {
            ...
            if(target.NeedBoxIndicator && isTargetVisible)
            {
                ...
                indicator.GetComponent<Button>().onClick.RemoveAllListeners();
                indicator.GetComponent<Button>().onClick.AddListener(delegate { MyButtonFunction(target); });
            }
            ...
            if (indicator)
            {
                ...
                if (target.NeedNameText) indicator.SetNameText(target.NameText, distanceFromCamera);
    ...
    public void MyButtonFunction(Target target)
    {
        //Debug.Log("click" + target.NameText);
        Camera.main.transform.DOMove(target.transform.position-new Vector3(0,-100,100), 1);
